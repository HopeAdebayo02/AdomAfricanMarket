...
function FreshProducePage() {
  const allProduce = getProductsByCategory('fresh-produce')

  useEffect(() => {
    document.title = 'Fresh Produce - Adom African Market Minneapolis'
    const metaDesc = document.querySelector('meta[name="description"]')
    if (metaDesc) {
      metaDesc.setAttribute('content', 'Browse all fresh produce at Adom African Market in Minneapolis. Yams, plantains, cassava, okra, and more authentic African vegetables and fruits.')
    }
    return () => {
      ...
      <div className="bg-stone-50 border-b border-stone-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="flex items-center gap-2 text-xs sm:text-sm text-stone-500">
            ...
            <span>/</span>
            <span className="text-stone-800">Fresh Produce</span>
          </nav>
        </div>
      </div>
      ...
      <section className="py-16 sm:py-20 lg:py-28 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10 sm:mb-14">
            ...
            <p className="text-stone-500 text-sm mt-4">{allProduce.length} items available</p>
          </div>
