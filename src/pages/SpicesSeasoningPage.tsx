...
function SpicesSeasoningPage() {
  const allSpices = getProductsByCategory('spices-seasoning')

  useEffect(() => {
    document.title = 'Spices & Seasoning - Adom African Market Minneapolis'
    const metaDesc = document.querySelector('meta[name="description"]')
    if (metaDesc) {
      metaDesc.setAttribute('content', 'Browse all spices and seasonings at Adom African Market in Minneapolis. Shrimp powder, alligator pepper, njansan, and more authentic African spices.')
    }
    return () => {
      ...
      <div className="bg-stone-50 border-b border-stone-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="flex items-center gap-2 text-xs sm:text-sm text-stone-500">
            ...
            <span>/</span>
            <span className="text-stone-800">Spices & Seasoning</span>
          </nav>
        </div>
      </div>
      ...
      <section className="py-16 sm:py-20 lg:py-28 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10 sm:mb-14">
            <p className="text-amber-600 text-xs sm:text-sm tracking-widest uppercase mb-3">Spices & Seasoning</p>
            ...
            <p className="text-stone-500 text-sm mt-4">{allSpices.length} items available</p>
          </div>
...
export default SpicesSeasoningPage
