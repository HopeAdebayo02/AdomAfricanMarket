...
function RiceGrainsPage() {
  const allItems = getProductsByCategory('rice-grains')

  useEffect(() => {
    document.title = 'Rice & Grains - Adom African Market Minneapolis'
    const metaDesc = document.querySelector('meta[name="description"]')
    if (metaDesc) {
      metaDesc.setAttribute('content', 'Browse all rice and grains at Adom African Market in Minneapolis. Parboiled rice, jasmine rice, garri, and more African staples.')
    }
    return () => {
      ...
      <div className="bg-stone-50 border-b border-stone-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="flex items-center gap-2 text-xs sm:text-sm text-stone-500">
            ...
            <span>/</span>
            <span className="text-stone-800">Rice & Grains</span>
          </nav>
        </div>
      </div>
