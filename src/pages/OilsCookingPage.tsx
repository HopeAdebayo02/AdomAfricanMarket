...
function OilsCookingPage() {
  const allItems = getProductsByCategory('oils-cooking')

  useEffect(() => {
    document.title = 'Oils & Cooking - Adom African Market Minneapolis'
    const metaDesc = document.querySelector('meta[name="description"]')
    if (metaDesc) {
      metaDesc.setAttribute('content', 'Browse all oils and cooking products at Adom African Market in Minneapolis. Palm oil, cooking oils, shea butter, and more.')
    }
    return () => {
      ...
      <div className="bg-stone-50 border-b border-stone-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="flex items-center gap-2 text-xs sm:text-sm text-stone-500">
            ...
            <span>/</span>
            <span className="text-stone-800">Oils & Cooking</span>
          </nav>
        </div>
      </div>
...
export default OilsCookingPage
